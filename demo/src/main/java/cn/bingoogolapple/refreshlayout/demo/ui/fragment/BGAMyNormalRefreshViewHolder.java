 ...
 * 描述:类似新浪微博下拉刷新风格
 */
public class BGAMyNormalRefreshViewHolder extends BGARefreshViewHolder {
     ...
     */
    public BGAMyNormalRefreshViewHolder(Context context, boolean isLoadingMoreEnabled) {
    ...
    private List<Integer> loadingList = new ArrayList<Integer>();

    private void loadLoading(){
        for(int i=1,len = loadLen+1;i<len;i++){
            int id = CommTool.getRID("icon_pull_down_"+i,"mipmap", App.getInstance());
            loadingList.add(id);
        }
    }
    ...
    public View getRefreshHeaderView() {
        if (mRefreshHeaderView == null) {
            mRefreshHeaderView = View.inflate(mContext, R.layout.view_refresh_header_normal_my, null);
    ...
    public void handleScale(float scale, int moveYDistance) {
        int flag = (int)(scale * 29f);
        Log.i("normal holder","scale_"+scale+"_moveYDistance_"+moveYDistance+"_flag_"+flag);
        mHeaderArrowIv.setImageResource(loadingList.get(flag));
...
//        mHeaderArrowIv.clearAnimation();
        ...
        // add by wangde
//        this.mHeaderChrysanthemumIv.setImageDrawable(null);
//        mHeaderChrysanthemumIv.setBackgroundResource(R.drawable.pull_down_loading_anim);
//        anim = (AnimationDrawable) mHeaderChrysanthemumIv.getBackground();
//        anim.start();
    }
